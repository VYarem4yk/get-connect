...
export const UPDATE_TEXT_MESSAGE_CREATOR = (text) => ({
  type: "TEXT-MESSAGE-CHANGE",
  newText: text,
});
export const SEND_MESSAGE_CREATOR = () => ({ type: "ADD-POST" });
...
export let store = {
  ...
  dispatch(action) {
    if (action.type === ADD_POST) {
      ...
      this._state.dialogsPage.curentMessageText = action.newMessage;
      this._callSubscriber(this._state);
    } else if (action.type === SEND_MESSAGE) {
      let newMessage = {
        id: "6",
        text: this._state.dialogsPage.curentMessageText,
        owner: "messageYour",
      };
      this._state.dialogsPage.DialogData.unshift(newMessage);
      this._state.dialogsPage.curentMessageText = "";
      this._callSubscriber(this._state);
    }
  },
};
